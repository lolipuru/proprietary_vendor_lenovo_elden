__kernel void DCFaceGeometryCaliY(__global uchar *img, __global float *input,
    __global float *output, int image_width, int image_height, int stride)
{
    ...
    if ((xf >= 0.0f) && (yf >= 0.0f) && (xf <= image_height - 1) && (yf <= image_width - 1)){
        ...
        luma1 = img[(x1 - 1) * stride + y1] * w1[x2] * w2[y2]
            + img[(x1 - 1) * stride + y1 + 1] * w1[x2] * w3[y2]
            + img[x1 * stride + y1 - 1] * w2[x2] * w1[y2]
            + img[x1 * stride + y1] * w2[x2] * w2[y2]
            + img[x1 * stride + y1 + 1] * w2[x2] * w3[y2]
            + img[x1 * stride + y1 + 2] * w2[x2] * w4[y2]
            + img[(x1 + 1) * stride + y1 - 1] * w3[x2] * w1[y2]
            + img[(x1 + 1) * stride + y1] * w3[x2] * w2[y2]
            + img[(x1 + 1) * stride + y1 + 1] * w3[x2] * w3[y2]
            + img[(x1 + 1) * stride + y1 + 2] * w3[x2] * w4[y2]
            + img[(x1 + 2) * stride + y1] * w4[x2] * w2[y2]
            + img[(x1 + 2) * stride + y1 + 1] * w4[x2] * w3[y2];

        output[y * width + x] = luma1 / W;
    }
