    ...
    int outStride)
{
    ...
    uchar pixel1 = srcPtr[y1 * inStride + x1];
    uchar pixel2 = srcPtr[y1 * inStride + x2];
    uchar pixel3 = srcPtr[y2 * inStride + x1];
    uchar pixel4 = srcPtr[y2 * inStride + x2];
    float top_pixel = pixel1 * (1 - x_diff) + pixel2 * x_diff;
    float bottom_pixel = pixel3 * (1 - x_diff) + pixel4 * x_diff;
    dstPtr[y * outStride + x] =
    ...
    int outStride)
{
    ...
    float y_diff = inputY - y1;
    uchar pixel1 = srcVUPtr[y1 * inStride + x1];
    uchar pixel2 = srcVUPtr[y1 * inStride + x2];
    uchar pixel3 = srcVUPtr[y2 * inStride + x1];
    uchar pixel4 = srcVUPtr[y2 * inStride + x2];
    float top_pixel = pixel1 * (1 - x_diff) + pixel2 * x_diff;
    float bottom_pixel = pixel3 * (1 - x_diff) + pixel4 * x_diff;
    dstVUPtr[y * outStride + (2*x+1)] =
        top_pixel * (1 - y_diff) + bottom_pixel * y_diff;
    ...
    int outStride)
{
    ...
    float top_pixel = pixel1 * (1 - x_diff) + pixel2 * x_diff;
    float bottom_pixel = pixel3 * (1 - x_diff) + pixel4 * x_diff;
    dstVUPtr[y * outStride + 2*x] =
        top_pixel * (1 - y_diff) + bottom_pixel * y_diff;

    return 0;
}

__kernel void BilinearInterpolationNV21(
    __global uchar *srcPtr,
    __global uchar *dstPtr,
    int srcWidth,
    int srcHeight,
    ...
    int outScanline)
{
    ...
    __global uchar *uvIn = srcPtr + inStride * inScanline;

    __global uchar *yOut = dstPtr;
    __global uchar *uvOut = dstPtr + outStride * outScanline;

    bilinearInterpolationPlaneY(yIn, yOut, x, y, srcWidth, srcHeight, dstWidth, dstHeight, inStride, outStride);
    bilinearInterpolationPlaneV(uvIn, uvOut, x, y,
        srcWidth / 2, srcHeight / 2, dstWidth / 2, dstHeight / 2, inStride, outStride);
