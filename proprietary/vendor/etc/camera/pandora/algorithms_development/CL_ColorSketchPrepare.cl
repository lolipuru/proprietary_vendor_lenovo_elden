// CL Kernel Source Code for ColorSketchPrepare Algorithm
// Version 1.0.0
...
__kernel void ColorSketchPrepare(
    ...
    __private const int datalength)
{
    ...
    float increment = 1.0f; // improve performance
    float weight    = 1.0f; // improve performance
    ...
    float rNew = luma * (1.0f - k) + r * k;
    float gNew = luma * (1.0f - k) + g * k;
    float bNew = luma * (1.0f - k) + b * k;
    ...
    pInputRGB[indexR] =
        rNew * (1 - weight) + (float)(gColorSketchLut[(int)(rNew + 0.5f)]) * weight;
    pInputRGB[indexG] =
        gNew * (1 - weight) + (float)(gColorSketchLut[(int)(gNew + 0.5f)]) * weight;
    pInputRGB[indexB] =
        bNew * (1 - weight) + (float)(gColorSketchLut[(int)(bNew + 0.5f)]) * weight;
