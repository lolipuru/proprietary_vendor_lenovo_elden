// CL Kernel Source Code for DefoggingBilinear Algorithm
// Version 1.0.0
...
__kernel void DefoggingBilinear(
    ...
    int datatype)
{
    ...
    if (u < width && v < height) {
      fptrin = input;
      fptrout = output;
      fptrout[index + dim * newDataLength] =
        (fptrin[y * width + x + dim * datalength] * (1.0f - dx)*(1.0f - dy) +
        fptrin[y_inc * width + x + dim * datalength] * (1.0f - dx) * dy +
        fptrin[y * width + x_inc + dim * datalength] * dx * (1.0f - dy) +
        fptrin[y_inc * width + x_inc + dim * datalength] * dx * dy);
    }
}