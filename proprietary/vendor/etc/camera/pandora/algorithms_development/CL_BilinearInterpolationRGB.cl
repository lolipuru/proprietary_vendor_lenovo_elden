// CL Kernel Source Code for BilinearInterpolationRGB Algorithm
// Version 1.0.0

__kernel void BilinearInterpolationRGB(
    ...
    int output_height)
{
    ...
    int pixel1 = input_image[y1 * input_width + x1 + dim * inSize];
    int pixel2 = input_image[y1 * input_width + x2 + dim * inSize];
    int pixel3 = input_image[y2 * input_width + x1 + dim * inSize];
    int pixel4 = input_image[y2 * input_width + x2 + dim * inSize];
    int top_pixel    = pixel1 * (1 - x_diff) + pixel2 * x_diff;
    int bottom_pixel = pixel3 * (1 - x_diff) + pixel4 * x_diff;
